...
export function mapExhibitionBooth(row: ExhibitionBoothRow): ExhibitionBooth {
  ...
  const derivedCategory =
    ...
    (company?.primaryIndustry
      ? getCategoryName(company.primaryIndustry, "en")
      : "General Pavilion");
  ...
  if (!derivedBoothNumber) {
    const numericPart =
      (parseInt(row.id.replace(/[^0-9]/g, "").slice(0, 3) || "1", 10) % 99) + 1;
...
export function mapExhibitionExhibit(
  row: ExhibitionItemRow,
): ExhibitionExhibit {
  return {
    id: row.id,
...
export function mapExhibitionDocument(
  row: ExhibitionDocumentRow,
): ExhibitionDocument {
  return {
    id: row.id,
    boothId: row.booth_id,
...
export function mapExhibitionApplication(
  row: ExhibitionApplicationRow,
): ExhibitionApplication {
  return {
    id: row.id,
    exhibitionId: row.exhibition_id,
...
export async function getExhibitions(): Promise<Exhibition[]> {
  try {
    const rows = await restGet<ExhibitionRow>(
      "exhibitions?select=*&order=start_date.asc",
    );
...
export async function getExhibitionBySlug(
  ...
  try {
    const rows = await restGet<ExhibitionRow>(
      `exhibitions?select=*&slug=eq.${encodeURIComponent(slug)}&limit=1`,
    );
 ...
 */
export async function getBoothsByExhibitionSlug(
  slug: string,
  ...
  try {
    ...
    console.warn(
      `[exhibitions-service] Failed to fetch booths for ${slug}:`,
      err,
    );
    return [];
  }
 ...
 */
export async function getBoothCompanyId(
  boothId: string,
): Promise<string | null> {
  try {
 ...
 */
export async function getExhibitBoothId(
  exhibitId: string,
): Promise<string | null> {
  try {
...
export async function getBoothDetails(
  ...
  try {
    ...
    console.warn(
      `[exhibitions-service] Failed to fetch booth details ${id}:`,
      err,
    );
    return null;
  }
...
export async function createExhibitionApplication(
  ...
  if (!cfg) {
    throw new Error(
      "Supabase is not configured — cannot submit an exhibition application.",
    );
  }
...
export async function getExhibitionApplicationById(
  ...
  try {
    ...
    console.warn(
      `[exhibitions-service] Failed to fetch application ID ${id}:`,
      err,
    );
    return null;
  }
...
export async function getExhibitionApplicationsByExhibitionId(
  ...
  try {
    ...
    console.warn(
      `[exhibitions-service] Failed to fetch applications for exhibition ${exhibitionId}:`,
      err,
    );
    return [];
  }
...
export async function checkDuplicateApplication(
  ...
  try {
    ...
    console.warn(
      "[exhibitions-service] Failed to check for duplicate applications:",
      err,
    );
    return false;
  }
...
export async function saveBoothDraft(
  ...
  if (data.shortDescription !== undefined)
    record.short_description = data.shortDescription;
  ...
  if (error || !row) {
    throw new Error(
      `Failed to update booth draft ${id}: ${error?.message || "no row returned"}`,
    );
  }
 ...
 */
export async function getExhibitsForBooth(
  boothId: string,
...
export async function getExhibitById(
  id: string,
): Promise<ExhibitionExhibit | null> {
  try {
    const rows = await restGet<ExhibitionItemRow>(
...
export async function createExhibit(
  ...
  if (error || !row) {
    throw new Error(
      `Failed to create exhibit: ${error?.message || "no row returned"}`,
    );
  }
...
export async function updateExhibit(
  ...
  if (data.isFeatured !== undefined)
    record.is_featured = Boolean(data.isFeatured);
  ...
  if (error || !row) {
    throw new Error(
      `Failed to update exhibit ${id}: ${error?.message || "no row returned"}`,
    );
  }
 ...
 */
export async function duplicateExhibit(
  id: string,
  client: SupabaseClient,
): Promise<ExhibitionExhibit> {
  const { data: srcRow, error: srcError } = await client
    ...
    .single();
  if (srcError || !srcRow)
    throw new Error(`Exhibit to duplicate not found: ${id}`);
  ...
  if (error || !row) {
    throw new Error(
      `Failed to duplicate exhibit: ${error?.message || "no row returned"}`,
    );
  }
...
export async function approveApplication(
  ...
  if (appError || !appRow) {
    throw new Error(
      `Failed to approve application ${id}: ${appError?.message || "no row returned"}`,
    );
  }

  const application = mapExhibitionApplication(
    appRow as ExhibitionApplicationRow,
  );

  // Automatically create an empty draft booth for the newly-approved exhibitor.
  const resolvedCompanyId =
    application.companyId || "00000000-0000-0000-0000-000000000000";
  ...
  if (boothError) {
    console.warn(
      "[exhibitions-service] Auto draft booth creation warning:",
      boothError.message,
    );
  }
...
export async function rejectApplication(
  ...
  if (error || !row) {
    throw new Error(
      `Failed to reject application ${id}: ${error?.message || "no row returned"}`,
    );
  }
...
export async function updateExhibition(
  ...
  if (error || !row) {
    throw new Error(
      `Failed to update exhibition ${id}: ${error?.message || "no row returned"}`,
    );
  }
...
export async function createExhibition(
  ...
  if (error || !row) {
    throw new Error(
      `Failed to create exhibition: ${error?.message || "no row returned"}`,
    );
  }
...
export async function updateExhibitionStatus(
  ...
  if (error || !row) {
    throw new Error(
      `Failed to update exhibition status ${id}: ${error?.message || "no row returned"}`,
    );
  }
...
export async function getOrganizerDashboardStats(
  ...
  const apps = await getExhibitionApplicationsByExhibitionId(
    exhibitionId,
    client,
  );
  ...
  const publishedBoothCount = booths.filter(
    (b) => b.status === "Published" && !b.isArchived,
  ).length;
...
export async function assignBoothDetails(
  ...
  if (error || !row) {
    throw new Error(
      `Failed to assign booth details for ${boothId}: ${error?.message || "no row returned"}`,
    );
  }
...
export async function updateBoothStatus(
  ...
  if (error || !row) {
    throw new Error(
      `Failed to update booth status for ${boothId}: ${error?.message || "no row returned"}`,
    );
  }
...
export async function loadStatistics(
  ...
  const draftBooths = booths.filter(
    (b) => (b.status === "Draft" || b.status === "Submitted") && !b.isArchived,
  ).length;
  const archivedBooths = booths.filter(
    (b) => b.isArchived || b.status === "Archived",
  ).length;
...
export async function updateApplicationReviewNotes(
  ...
  if (error || !row) {
    throw new Error(
      `Failed to update review notes for ${id}: ${error?.message || "no row returned"}`,
    );
  }
...
export function getRecentlyViewedStore(): ExhibitionRecentlyViewed[] {
  if (typeof globalThis !== "undefined") {
    ...
    if (!g.__recentlyViewedStore)
      g.__recentlyViewedStore = RECENTLY_VIEWED_STORE;
...
function mapFavoriteRow(row: ExhibitionFavoriteRow): ExhibitionFavorite {
  return {
    ...
    targetId: (row.target_type === "booth"
      ? row.booth_id
      : row.exhibit_id) as string,
...
export async function getFavorites(
  ...
  return getFavoritesWithRelations(
    (data as ExhibitionFavoriteRow[]).map(mapFavoriteRow),
  );
}

async function getFavoritesWithRelations(
  favorites: ExhibitionFavorite[],
): Promise<ExhibitionFavorite[]> {
  const boothIds = favorites
    .filter((f) => f.targetType === "booth")
    .map((f) => f.targetId);
  const exhibitIds = favorites
    .filter((f) => f.targetType === "exhibit")
    .map((f) => f.targetId);
...
export async function addFavorite(
  ...
  const [withRelations] = await getFavoritesWithRelations([
    mapFavoriteRow(row),
  ]);
...
): { from: Date; to: Date; labels: string[] } {
  const to = new Date();
  let from = new Date();
  let labels: string[] = [];
...
export async function getOrganizerAnalytics(
  ...
  try {
    ...
    console.warn(
      "[exhibitions-service] getOrganizerAnalytics booth fetch failed:",
      err,
    );
  }
  ...
  const service = getServiceClient();
  const apps = service
    ? await getExhibitionApplicationsByExhibitionId(exhibitionId, service)
    : [];
  const pendingApplications = apps.filter((a) => a.status === "Pending").length;
    ...
    filter === "today"
      ...
      : filter === "7days"
        ...
        : filter === "30days"
          ? 25
          : 12;
  const totalVisitors = hasActivity
    ? Math.floor(250 + seed() * 500) * multiplier
    : 0;
  const uniqueVisitors = hasActivity
    ? Math.floor(150 + seed() * 300) * multiplier
    : 0;
  const totalMeetings = hasActivity
    ? Math.floor(10 + seed() * 25) * multiplier
    : 0;
  const completedMeetings = hasActivity
    ? Math.floor(totalMeetings * (0.6 + seed() * 0.2))
    : 0;
  const totalRfqs = hasActivity ? Math.floor(15 + seed() * 30) * multiplier : 0;
  const totalCatalogDownloads = hasActivity
    ? Math.floor(40 + seed() * 80) * multiplier
    : 0;
  const qrScans = hasActivity ? Math.floor(30 + seed() * 60) * multiplier : 0;
  const averageSessionDuration = hasActivity
    ? Math.floor(180 + seed() * 360)
    : 0;
  ...
  const topPerformingBooths = booths
    .map((b) => {
  ...
  const topCategories = Object.entries(categoryCounts)
    .map(([name, count]) => ({
      name,
      count,
      percentage: Math.round((count / Math.max(1, totalBooths)) * 100),
    }))
...
export async function getExhibitorAnalytics(
  ...
  const rejectedMeetings = Math.floor(
    (meetingRequests - acceptedMeetings) * 0.5,
  );
  ...
  const exhibitsPerformance = [
    {
      id: "ex-1",
      name: "Premium Sfax Extra Reserve Olive Oil",
      views: Math.floor(80 + seed() * 150) * multiplier,
      downloads: Math.floor(10 + seed() * 40) * multiplier,
    },
    {
      id: "ex-2",
      name: "Organic Date Syrup (Pure Extract)",
      views: Math.floor(50 + seed() * 100) * multiplier,
      downloads: Math.floor(5 + seed() * 25) * multiplier,
    },
    {
      id: "ex-3",
      name: "Hand-crafted Ceramic Serving Trays",
      views: Math.floor(30 + seed() * 60) * multiplier,
      downloads: Math.floor(2 + seed() * 15) * multiplier,
    },
  ];
...
export async function getTrafficReport(
  ...
  const byDay = labels.map((label) => {
    ...
    const views = Math.floor(
      (totalViews / labels.length) * (0.7 + dSeed() * 0.6),
    );
...
export async function getDownloadReport(
  ...
  const byDocument = [
    {
      id: "doc-1",
      name: "Official B2B Export Catalogue 2026.pdf",
      count: Math.floor(totalDownloads * 0.5),
    },
    {
      id: "doc-2",
      name: "ISO 22000 & Organic Certifications.pdf",
      count: Math.floor(totalDownloads * 0.3),
    },
    {
      id: "doc-3",
      name: "Sahara Dates - Export Specs & Logistics.pdf",
      count: Math.floor(totalDownloads * 0.2),
    },
  ];
  ...
  const byBooth = [
    {
      ...
      count: Math.floor(totalScans * 0.45),
    },
    {
      id: "booth-sahara",
      boothNumber: "A-02",
      companyName: "Sahara Dates Export",
      count: Math.floor(totalScans * 0.3),
    },
    {
      id: "booth-carthage",
      boothNumber: "B-15",
      companyName: "Carthage Textiles",
      count: Math.floor(totalScans * 0.25),
    },
  ];
...
export async function getRecentlyViewed(
  visitorId: string,
): Promise<ExhibitionRecentlyViewed[]> {
  const list = getRecentlyViewedStore().filter(
...
async function attachBooths(
  meetings: ExhibitionMeeting[],
  ...
  return meetings.map((m) => ({
    ...m,
    booth: boothsById.get(m.boothId) || null,
  }));
}
...
export async function createMeeting(
  ...
  input: Omit<
    ExhibitionMeeting,
    "id" | "visitorId" | "status" | "createdAt" | "updatedAt"
  >,
): Promise<ExhibitionMeeting> {
  ...
  const [withBooth] = await attachBooths([
    mapMeetingRow(data as ExhibitionMeetingRow),
  ]);
  return withBooth;
}
...
export async function updateMeetingStatus(
  ...
  const { data, error } = await service
    ...
    .single();
  if (error || !data)
    throw new Error(error?.message || `Meeting not found: ${meetingId}`);
