...
export function StoreRfqButton({
  companyId,
  companyName,
}: {
  companyId: string;
  ...
  const dict = t.rfq.store;

  const [open, setOpen] = useState(false);
  const [values, setValues] = useState<RfqInput>(EMPTY_RFQ);
  const [errors, setErrors] = useState<
    Partial<Record<RfqField, "required" | "email" | "phone" | "message">>
  >({});
            ...
            {status === "success" ? (
              ...
              <form onSubmit={handleSubmit} className="p-5 space-y-3">
                ...
                <div>
                  ...
                  />
                  {errText("companyName") && (
                    <span className="text-[10px] text-destructive block font-bold mt-1">
                      {errText("companyName")}
                    </span>
                  )}
                </div>
                  ...
                  <div>
                    <input
                      ...
                      className={inputClass(
                        Boolean(errors.deliveryDestination),
                      )}
                    />
                    {errText("deliveryDestination") && (
                      <span className="text-[10px] text-destructive block font-bold mt-1">
                        {errText("deliveryDestination")}
                      </span>
                    )}
                ...
                <div>
                  <input
                    ...
                    onChange={(e) =>
                      setValues({ ...values, productRequested: e.target.value })
                    }
                    className={inputClass(Boolean(errors.productRequested))}
                  />
                  {errText("productRequested") && (
                    <span className="text-[10px] text-destructive block font-bold mt-1">
                      {errText("productRequested")}
                    </span>
                  )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setValues({ ...values, targetPrice: e.target.value })
                    }
